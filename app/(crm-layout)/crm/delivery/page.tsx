  ...
  const fetchBoards = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const deliveryBoard = boardsList.find(
          (b: CRMBoard) =>
            b.title.toLowerCase().includes("delivery") ||
            b.title.toLowerCase().includes("fulfillment"),
        );
        if (deliveryBoard) {
          setSelectedBoardId(deliveryBoard.id);
        } else if (boardsList.length > 1) {
          setSelectedBoardId(boardsList[1].id);
        }
  ...
  return (
    <div className="p-8 min-h-screen bg-gradient-to-br from-indigo-950 via-zinc-950 to-purple-950">
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">Delivery Board</h1>
          <p className="text-zinc-400">
            {selectedBoard?.title || "Delivery"} - Track order fulfillment
          </p>
        </div>

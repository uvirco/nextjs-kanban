...
export async function POST(request) {
  try {
    const { host = 'imap.gmail.com', port = 993, user = process.env.GMAIL_USER, pass = process.env.GMAIL_PASS, tls = true } = await request.json(); // Use env vars as defaults

    console.log('IMAP Config:', { host, port, user: user ? 'set' : 'not set', pass: pass ? 'set' : 'not set', tls });
    ...
    const fetchOptions = { bodies: [''], struct: true, envelope: true, markSeen: false };
    ...
    connection.end();
    return NextResponse.json({ message: `${emailsProcessed} emails processed` }, { status: 200 });
  } catch (error) {
